<script lang="ts">
	...
	import type { CallType, IncomingCallEvent } from '$lib/utils/chat/webrtc';
	import {
		buildReplySnippet,
	...
	import { readSessionRoomPreferences, writeSessionRoomPreferences } from '$lib/utils/sessionPreferences';
	import {
		closeGlobalSocket,
		globalMessages,
		initGlobalSocket,
	...
	type WebRTCManagerModule = typeof import('$lib/utils/chat/webrtc');
	type WebRTCManagerInstance = import('$lib/utils/chat/webrtc').WebRTCManager;
	...
	const TEMP_ALLOWED_WORKSPACE_MODULES: WorkspaceModule[] = ['dashboard'];
	...
	function isTempWorkspaceModuleAllowed(module: WorkspaceModule) {
		return TEMP_ALLOWED_WORKSPACE_MODULES.includes(module);
	}
	...
	function syncSessionChatLayoutPreferencesFromStorage() {
		if (!browser) {
			return { onlinePanelCollapsed: false };
		}
		try {
			const stored = window.sessionStorage.getItem('temp_chat_online_panel_collapsed');
			const nextValue = stored === '1';
			isOnlinePanelCollapsed = nextValue;
			return { onlinePanelCollapsed: nextValue };
		} catch {
			isOnlinePanelCollapsed = false;
			return { onlinePanelCollapsed: false };
		}
	}
	...
	let webrtcManager: WebRTCManagerInstance | null = null;
	let webrtcManagerModulePromise: Promise<WebRTCManagerModule> | null = null;
	...
	function syncActiveRoomPasswordFromHash() {
		...
		if (!pathname.startsWith('/temp/chat/')) {
			return;
		}
	...
	async function silentlyJoinRoomAsMember(targetRoomId: string, userId: string, username: string) {
		...
		try {
			...
			await goto(`/temp/chat/${encodeURIComponent(joinedRoomId)}?${params.toString()}`, {
				replaceState: true,
				noScroll: true,
				keepFocus: true
			});
	...
	function syncCallStreamsFromManager() {
		...
		remoteCallStreams = webrtcManager
			...
			.filter((entry: CallStreamSlot) => streamHasLiveTracks(entry.stream));
	}
	...
	async function loadWebRTCManagerModule() {
		if (!webrtcManagerModulePromise) {
			webrtcManagerModulePromise = import('$lib/utils/chat/webrtc');
		}
		return webrtcManagerModulePromise;
	}

	async function ensureWebRTCManager() {
		...
		if (!webrtcManager) {
			const { WebRTCManager } = await loadWebRTCManagerModule();
	...
	function getCallInviteTargetUserIds() {
		...
		const connected = new Set(
			webrtcManager.getPeerUserIds().map((entry: string) => normalizeIdentifier(entry))
		);
	...
	function selectRoom(targetRoomId: string, memberState: boolean, focusMsgID = '') {
		...
		void goto(`/temp/chat/${encodeURIComponent(normalizedTargetRoomId)}${query ? `?${query}` : ''}`);
	}
	...
	function openWorkspaceModule(
		...
		if (!isTempWorkspaceModuleAllowed(module)) {
			return false;
		}
	...
	function toggleOnlinePanelCollapse() {
		...
		try {
			window.sessionStorage.setItem(
				'temp_chat_online_panel_collapsed',
				isOnlinePanelCollapsed ? '1' : '0'
			);
		} catch {
			// Ignore storage write errors for the lightweight temp chat route.
		}
	}
	...
	async function renameRoom(targetRoomId: string = roomId) {
		...
		try {
			...
			if (normalizedRoomID === roomId) {
				...
				await goto(`/temp/chat/${encodeURIComponent(normalizedRoomID)}?${params.toString()}`, {
					replaceState: true,
					noScroll: true,
					keepFocus: true
				});
	...
	async function createRoomFromMenu() {
		showLeftMenu = false;
		showErrorToast('Room creation is not available in temp chat.');
	}
	...
	async function joinCurrentRoom() {
		...
		try {
			while (true) {
				...
				await goto(`/temp/chat/${encodeURIComponent(roomId)}?${params.toString()}${passwordHash}`);
				return;
			}
		} catch (error) {
	...
	function toggleBreakSelectionMode() {
		...
		showErrorToast('Branching is not available in temp chat.');
	}
	...
	async function createBreakRoom(message: ChatMessage) {
		...
		try {
			...
			const passwordHash = buildRoomPasswordHash($activeRoomPassword);
			await goto(`/temp/chat/${encodeURIComponent(breakRoomId)}?${params.toString()}${passwordHash}`);
			return true;
		} catch (error) {
...
>
	<div class="sidebar-pane">
		<ChatSidebar
			...
			showCreateRoomAction={false}
			showRelationsMapAction={false}
			showBranchBadges={false}
			bind:chatListSearch
		...
		{#if !isCanvasFullscreen}
			<section class="chat-window">
				<ChatRoomHeader
					...
					showBreakSelectionAction={false}
				...
				<div class="chat-window-shell" class:is-expired={isRoomExpired}>
					<ChatWindow
						...
						hiddenContextActions={['pin', 'branch']}
