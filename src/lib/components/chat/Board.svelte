<script lang="ts">
	...
	const THEME_ADAPTIVE_LIGHT_INK_CANDIDATES = new Set([
		THEME_ADAPTIVE_LIGHT_INK,
		'#000000',
		'#0f172a',
		'#111111',
		'#1f2937'
	]);
	const THEME_ADAPTIVE_DARK_INK_CANDIDATES = new Set([
		THEME_ADAPTIVE_DARK_INK,
		'#ffffff',
		'#fafafa',
		'#f3f4f6',
		'#e5e7eb'
	...
	function resolveThemeAwareInkColor(color: string) {
		...
		if (
			THEME_ADAPTIVE_LIGHT_INK_CANDIDATES.has(normalizedHex) ||
			THEME_ADAPTIVE_DARK_INK_CANDIDATES.has(normalizedHex)
		) {
		...
		const isNearNeutral = channelSpread <= 28;
		const isNearWhite = brightness >= 210;
		const isNearBlack = brightness <= 64;
