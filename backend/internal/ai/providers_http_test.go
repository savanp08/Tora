...
func TestBuildDefaultProvidersFromEnvPrioritizesVertex(t *testing.T) {
	...
	if _, ok := providers[2].(*MistralProvider); !ok {
		t.Fatalf("expected third provider to be MistralProvider, got %T", providers[2])
	}
	if _, ok := providers[3].(*GroqProvider); !ok {
		t.Fatalf("expected fourth provider to be GroqProvider, got %T", providers[3])
	}
}
